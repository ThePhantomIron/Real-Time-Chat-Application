...
from ..core.speech import SpeechToTextError, SpeechToTextUnavailable, transcribe_once
...
class ChatWindow(BaseWindow):
    """
    Full chat UI: topbar, sidebar, message canvas, input bar.
    Handles all incoming server messages and user actions.
    """
    ...
    def __init__(self, root: tk.Tk, session: UserSession, client: Client, on_logout=None):
        ...
        client._on_err = lambda _: root.after(
            0, lambda: self._status("Disconnected", Theme.DANGER)
        )
    ...
    def _build(self):
        ...
        self._status("Connected", Theme.ONLINE)
    ...
    def _voice_worker(self):
        try:
            ...
            self.root.after(0, lambda reason=str(exc): self._voice_unavailable(reason))
            return
        except SpeechToTextError as exc:
            self.root.after(0, lambda reason=str(exc): self._voice_failed(reason))
            return

        self.root.after(0, lambda text=transcript: self._apply_transcript(text))

    def _apply_transcript(self, transcript: str):
        self._set_mic_busy(False)
        current = self._msg_var.get().strip()
        combined = f"{current} {transcript}".strip() if current else transcript.strip()
        if len(combined) > Config.MAX_MSG:
            combined = combined[:Config.MAX_MSG]
            messagebox.showwarning(
                "Voice input",
                f"Transcript was trimmed to {Config.MAX_MSG} characters.",
            )
        self._msg_var.set(combined)
        if self._entry:
            self._entry.icursor(len(combined))
            self._entry.focus_set()

    def _voice_unavailable(self, reason: str):
        self._set_mic_busy(False)
        messagebox.showwarning("Mic unavailable", reason)
        if self._entry:
            self._entry.focus_set()

    def _voice_failed(self, reason: str):
        self._set_mic_busy(False)
        messagebox.showwarning("Voice input", reason)
        if self._entry:
            self._entry.focus_set()

    def _set_mic_busy(self, busy: bool):
        self._mic_busy = busy
        if self._mic_btn:
            self._mic_btn.config(
                text="🎤 Listening" if busy else "🎤",
                bg=Theme.ACCENT2 if busy else Theme.BG_LIGHT,
                fg="#0D1117" if busy else Theme.TEXT,
                activebackground=Theme.ACCENT2 if busy else Theme.BG_LIGHT,
                activeforeground="#0D1117" if busy else Theme.TEXT,
    ...
    def _add_ch(self, name: str):
        ...
        if not self._cm.can_add(name):
            messagebox.showwarning("Cannot create group", "Invalid, duplicate, or limit reached.")
            return
    ...
    def _render_stream(self, key: str):
        ...
        for item in self._cm.history(key):
            ...
            if self._cm.is_dm(key) and is_self and item.get("id") is not None:
                peer = key[len("dm:"):]
                on_delete = lambda msg_id=item["id"], dm_peer=peer: self._delete_dm_message(msg_id, dm_peer)
            Bubbles.chat(
                self._canvas.frame,
                username,
                item["text"],
